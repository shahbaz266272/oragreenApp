...
export async function handleUserLogin(email, password) {
  try {
    ...
    const { getPushToken, registerForPushNotifications } = require('../services/notificationService');
    
    const pushToken = await getPushToken() || await registerForPushNotifications();
...
async function sendPromotionToAllUsers(promotion) {
  try {
        ...
        sendExpoNotification(
          ...
          if (result.success) successCount++;
            ...
            if (result.error?.includes('InvalidRegistration')) {
              User.updateOne({ pushToken: user.pushToken }, { pushToken: null });
            }
          }
        })
      );
    ...
    console.log(`Promotion sent: ${successCount} success, ${failureCount} failed`);
...
app.post('/api/user/push-token', async (req, res) => {
  try {
    ...
    if (!pushToken.startsWith('ExponentPushToken') && !pushToken.startsWith('FCM:')) {
